...
public class Arrays
{
	...
	static void Example_2()
	{
		...
		for(int i = 0; i < array.length; i++)
		{
			...
			System.out.println("arr[" + i + "]=" + array2[i]);
		}
	...
	static void Your_Own_Code() 
	{
		// Write new methods min, max, and count_dupes.
				...
				//  After writing your code, to test, uncomment this section as needed.
				 	...
				 	System.out.println("This array has " + count_dupes(rand_array) + " number of 1's in the array.");
				 
	}
	public static int max(int [] rand_array) 
	{
		...
		public static int min(int [] rand_array) 
		{
			int min = rand_array[0];
			for (int i = 0; i<rand_array.length; i++) 
			{
				if (rand_array[i] < min ) 
				{
		...
		public static int count_dupes(int [] rand_array)
		{
			int j = 0;
			int count_dupes = rand_array[0]; 
			for (int i = j+1; i<rand_array.length; i++) 
			{
				if ( 1 == count_dupes) 
				{
					j++;
